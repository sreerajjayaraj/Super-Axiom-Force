...
// Fallback offsets for Crimson Desert 1.0.0.1573.
// Range/PullSpeed still self-scan at runtime if these data RVAs move.
static const uintptr_t kAxiomRangeOffset_Fallback = 0x5E9EB28;
static const uintptr_t kAxiomPullSpeedOffset_Fallback = 0x5E9F668;
static const uintptr_t kAerialManeuverOffset_Fallback = 0xE91C3D;
static const uintptr_t kActivationDelayOffset_Fallback = 0x44605D;
static const uintptr_t kReelDurationOffset_Fallback = 0x446CD2;
static const uintptr_t kPropellDurationOffset_Fallback = 0x447394;
...
static void DiscoverAddresses() {
    ...
    // StraightPull 1.06 call-sites:
    // E8 ?? D1 FF FF 48 83 EB 01 75 C8
    {
        ...
        // Compatibility: Improved Axiom Behavior may patch the same CALL before this
        // mod initializes. In that case the first five bytes are already NOPs, but
        // the following loop-control bytes are still stable.
    ...
    // Range/PullSpeed are the core Super Axiom Force feature. They live in runtime
    // game data, so bind only after checking that both target addresses are writable.
    {
        ...
        if (AddressInMainImage(reinterpret_cast<uintptr_t>(range), sizeof(float)) &&
            ...
            *pull == kDefaultPullSpeed) {
            ...
            char msg[256];
            sprintf_s(msg,
                      "Range/PullSpeed: binding enabled range_rva=0x%llX pull_rva=0x%llX range_current=%g pull_current=%g image_size=0x%llX",
            ...
            char msg[256];
            sprintf_s(msg,
                      "Range/PullSpeed: fallback offsets unavailable or stale; scanning image. fallback_range=%g fallback_pull=%g image_base=0x%llX image_size=0x%llX",
            ...
            if (FindRangePullPair(&g_pAxiomRange, &g_pAxiomPullSpeed)) {
                char scanMsg[256];
                sprintf_s(scanMsg,
                          "Range/PullSpeed: scan binding enabled range_rva=0x%llX pull_rva=0x%llX range_current=%g pull_current=%g",
                          RvaOf(g_pAxiomRange),
...
static bool ApplyAxiomRange(float range) {
    if (!g_pAxiomRange) {
        Log("WARNING: Range feature unavailable");
        return false;
    }

    const float current = *g_pAxiomRange;
    ...
    if (!VirtualProtect(g_pAxiomRange, sizeof(float), PAGE_EXECUTE_READWRITE, &oldProtect)) {
        ...
        MEMORY_BASIC_INFORMATION mbi{};
        if (QueryMemoryForLog(g_pAxiomRange, mbi)) {
            Logf("ERROR: Range memory protection failed rva=0x%llX current=%g target=%g state=0x%lX protect=%s(0x%lX) region_size=0x%llX error=%lu",
                 RvaOf(g_pAxiomRange),
                 current,
                 range,
                 mbi.State,
            ...
            Logf("ERROR: Range memory protection failed rva=0x%llX current=%g target=%g query_failed error=%lu",
                 RvaOf(g_pAxiomRange),
                 current,
                 range,
                 error);
    ...
    *g_pAxiomRange = range;
    ...
    Logf("Range applied rva=0x%llX previous=%g target=%g old_protect=%s(0x%lX)",
         RvaOf(g_pAxiomRange),
         current,
         range,
